...
Section code.
  ...
  Definition impl_append (global_append : loc): function := {|
    ...
    f_code := (
      <[ "#0" :=
        locinfo: loc_22 ;
        if{IntOp i32, None}: LocInfoE loc_22 ((LocInfoE loc_23 (use{PtrOp} (LocInfoE loc_25 (!{PtrOp} (LocInfoE loc_26 ("l")))))) ={PtrOp, PtrOp, i32} (LocInfoE loc_27 (NULL)))
        then
        locinfo: loc_4 ;
          Goto "#1"
        else
  ...
  Definition impl_partition (global_partition : loc): function := {|
    ...
    f_code := (
      <[ "#0" :=
        locinfo: loc_90 ;
        if{IntOp i32, None}: LocInfoE loc_90 ((LocInfoE loc_91 (use{PtrOp} (LocInfoE loc_93 (!{PtrOp} (LocInfoE loc_94 ("l")))))) ={PtrOp, PtrOp, i32} (LocInfoE loc_95 (NULL)))
        then
        locinfo: loc_32 ;
          Goto "#1"
        else
      ...
      <[ "#2" :=
        ...
        locinfo: loc_62 ;
        if{IntOp i32, None}: LocInfoE loc_62 ((LocInfoE loc_63 (use{IntOp i32} (LocInfoE loc_64 ("pivot")))) ≤{IntOp i32, IntOp i32, i32} (LocInfoE loc_65 (use{IntOp i32} (LocInfoE loc_66 ((LocInfoE loc_67 (!{PtrOp} (LocInfoE loc_69 (!{PtrOp} (LocInfoE loc_70 ("l")))))) at{struct_list_node} "val")))))
        then
        locinfo: loc_39 ;
          Goto "#3"
        else
  ...
  Definition impl_quicksort (global_append global_partition global_quicksort : loc): function := {|
    ...
    f_code := (
      <[ "#0" :=
        locinfo: loc_139 ;
        if{IntOp i32, None}: LocInfoE loc_139 ((LocInfoE loc_140 (use{PtrOp} (LocInfoE loc_142 (!{PtrOp} (LocInfoE loc_143 ("l")))))) ={PtrOp, PtrOp, i32} (LocInfoE loc_144 (NULL)))
        then
        locinfo: loc_100 ;
          Goto "#1"
        else
