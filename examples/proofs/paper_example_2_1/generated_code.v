...
Section code.
  ...
  Definition loc_35 : location_info := LocationInfo file_1 29 2 29 40.
  ...
  Definition loc_80 : location_info := LocationInfo file_1 41 2 41 40.
  ...
  Definition impl_alloc_from_start : function := {|
    ...
    f_code := (
      <[ "#0" :=
        locinfo: loc_81 ;
        if{IntOp i32, Some "#1"}: LocInfoE loc_81 ((LocInfoE loc_82 (use{IntOp size_t} (LocInfoE loc_83 ("sz")))) >{IntOp size_t, IntOp size_t, i32} (LocInfoE loc_84 (use{IntOp size_t} (LocInfoE loc_85 ((LocInfoE loc_86 (!{PtrOp} (LocInfoE loc_87 ("d")))) at{struct_mem_t} "len")))))
        then
        locinfo: loc_78 ;
          Goto "#2"
        else
