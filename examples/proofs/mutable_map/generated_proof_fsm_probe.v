...
Section proof_fsm_probe.
  ...
  Proof.
    ...
    prepare_parameters (m mp items key count).
    split_blocks ((
      <[ "#1" :=
        ∃ offset : nat,
    ...
    all: try by apply: probe_ref_go_next_take => // ? /lookup_rotate_r_Some ?; try lia; simplify_eq; naive_solver.
    all: print_sidecondition_goal "fsm_probe".
    Unshelve. all: try done; try apply: inhabitant; print_remaining_shelved_goal "fsm_probe".
  Qed.
End proof_fsm_probe.
