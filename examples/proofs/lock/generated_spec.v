...
Section spec.
  ...
  Global Instance lock_test_simplify_goal_place_inst_generated l_ β_ patt__:
    SimplifyGoalPlace l_ β_ (patt__ @ lock_test)%I (Some 100%N) :=
    λ T, i2p (simplify_goal_place_eq l_ β_ _ _ T (lock_test_unfold _)).

  Global Program Instance lock_test_simplify_hyp_val_inst_generated v_ patt__:
    SimplifyHypVal v_ (patt__ @ lock_test)%I (Some 100%N) :=
    λ T, i2p (simplify_hyp_val_eq v_ _ _ (lock_test_unfold _) T _).
  Next Obligation. done. Qed.
  Global Program Instance lock_test_simplify_goal_val_inst_generated v_ patt__:
