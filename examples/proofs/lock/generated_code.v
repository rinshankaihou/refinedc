...
Section code.
  ...
  Definition loc_164 : location_info := LocationInfo file_1 91 4 94 5.
  ...
  Definition impl_increment (global_sl_lock global_sl_unlock : loc): function := {|
    ...
    f_code := (
      <[ "#0" :=
        ...
        locinfo: loc_165 ;
        if{IntOp i32, Some "#1"}: LocInfoE loc_165 ((LocInfoE loc_166 (use{IntOp size_t} (LocInfoE loc_167 ((LocInfoE loc_168 ((LocInfoE loc_169 (!{PtrOp} (LocInfoE loc_170 ("t")))) at{struct_lock_test} "locked_struct")) at{struct_lock_test_inner} "b")))) >{IntOp size_t, IntOp size_t, i32} (LocInfoE loc_171 (UnOp (CastOp $ IntOp size_t) (IntOp i32) (LocInfoE loc_172 (i2v 0 i32)))))
        then
        locinfo: loc_140 ;
          Goto "#2"
        else
