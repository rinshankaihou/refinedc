...
Section proof_btree_member.
  ...
  Proof.
    ...
    + eapply (btree_invariant_in_range_child H2); naive_solver lia.
    + eapply (btree_invariant_in_range_child H2); naive_solver lia.
    + rewrite H1; apply: (btree_invariant_lookup_child H2); by naive_solver.
    + do 2 rewrite list_insert_id //; by destruct y0.
    + assert (k ∉ x0) as Hx1. { move => /H7 Hk. apply lookup_lt_Some in Hk. lia. } eapply (btree_invariant_in_range_child H2); by naive_solver.
    + assert (k ∉ x0) as Hx1. { move => /H7 Hk. apply lookup_lt_Some in Hk. lia. } eapply (btree_invariant_in_range_child H2); by naive_solver.
    ...
    Unshelve. all: try done; try apply: inhabitant; print_remaining_shelved_goal "btree_member".
  Qed.
