...
Proof.
  ...
  move => /Z.bits_inj_iff' Ha [Hb' /Z_bounded_iff_bits_nonneg Hb].
  apply Z.bits_inj_iff' => n' ?. rewrite Z.land_spec Z_ones_spec //.
  case_bool_decide; rewrite ?andb_true_r ?andb_false_r.
  2: { symmetry. naive_solver lia. }
  rewrite Z.add_nocarry_lxor.
  - rewrite Z.lxor_spec.
    move: (Ha n'). rewrite Z.land_spec Z_ones_spec // Z.bits_0. case_bool_decide => //.
    rewrite andb_true_r => -> //=. by case_match.
  - apply Z.bits_inj_iff' => n'' ?. rewrite Z.land_spec Z.bits_0.
    destruct (decide (n ≤ n'')).
    + rewrite Hb ?andb_false_r //; lia.
    + move: (Ha n''). rewrite Z.land_spec Z_ones_spec // Z.bits_0. case_bool_decide; [|lia].
...
Proof.
  move => [??]. rewrite !Z.shiftl_1_l -Z.land_ones // Z.sub_1_r -Z.ones_equiv.
  move => [? /Z_bounded_iff_bits_nonneg Ha] /Z.bits_inj_iff' Ha' [? /Z_bounded_iff_bits_nonneg Hb].
  ...
  rewrite Z_lunot_spec; [|lia]. rewrite Z_ones_spec //. case_bool_decide; rewrite ?andb_true_r ?andb_false_r.
  - move: (Ha' n'). rewrite Z.land_spec Z_ones_spec // Z.bits_0. case_bool_decide; [|lia].
    by rewrite andb_true_r => ->.
...
Proof.
  ...
  move => [? /Z_bounded_iff_bits_nonneg Ha] /Z.bits_inj_iff' Ha' [? /Z_bounded_iff_bits_nonneg Hc].
  apply Z.bits_inj_iff' => n'' ?. rewrite Z.land_spec Z.bits_0.
  destruct (decide (n ≤ n'')).
