...
Section proof___find_buddy.
  ...
  Proof.
    ...
    prepare_parameters (pool pageloc page vmemmap pages npages range_start_idx order max_order).
    split_blocks ((
      ∅
    ...
    all: try by eapply (find_buddy_result' page range_start_idx order npages); solve_goal.
    all: try by eapply find_buddy_result_eq; solve_goal.
    all: print_sidecondition_goal "__find_buddy".
    Unshelve. all: try done; try apply: inhabitant; print_remaining_shelved_goal "__find_buddy".
  Qed.
