...
let run : config -> string -> unit = fun cfg c_file ->
  ...
  let code_file = Filename.concat output_dir "code.v" in
  let spec_file = Filename.concat output_dir "spec.v" in
...
let _ =
  let cmds =
    [ init_cmd ; cpp_cmd ; ail_cmd ; check_cmd ; help_cmd ; version_cmd ]
  in
