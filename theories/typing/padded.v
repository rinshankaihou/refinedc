...
Section padded.
  ...
  Program Definition padded (ty : type) (lyty ly : layout) : type := {|
    ty_own β l :=
      ⌜lyty ⊑ ly⌝ ∗ ⌜l `has_layout_loc` ly⌝ ∗ l ◁ₗ{β} ty ∗ (l +ₗ lyty.(ly_size)) ◁ₗ{β} uninit (ly_offset ly lyty.(ly_size));
  |}%I.
  Next Obligation.
  ...
  Qed.
  Next Obligation.
    iIntros (ty lyty ly ? Heq l). iDestruct 1 as ([??] ?) "[Hl Hpad]".
  ...
  Proof.
    ...
    do 2 iSplit => //. iFrame.
    iExists []. rewrite heap_mapsto_own_state_nil. iPureIntro. split_and! => //.
    - rewrite /has_layout_val/ly_offset/ly_size /=. lia.
    - by apply: has_layout_ly_offset.
  Qed.
  ...
  Proof.
    iIntros "HT". iDestruct 1 as ([? ?] ?) "(Hl & Hr)".
  ...
  Proof.
    ...
    - iSplit => //. iSplit; first by iPureIntro; apply: has_layout_loc_trans. iExists _. iFrame. iPureIntro.
      split; last by apply has_layout_ly_offset.
  ...
  Lemma type_add_padded v2 β ly lyty ty T (p : loc) (n : Z):
    (⌜0 ≤ n⌝ ∗ ⌜Z.to_nat n ≤ ly.(ly_size)⌝%nat ∗ ⌜lyty.(ly_size) ≤ Z.to_nat n⌝%nat ∗ (p ◁ₗ{β} padded ty lyty (ly_set_size ly (Z.to_nat n)) -∗ v2 ◁ᵥ n @ int size_t -∗
          T (val_of_loc (p +ₗ n)) (t2mt ((p +ₗ n) @ &frac{β} (uninit (ly_offset ly (Z.to_nat n))))))) -∗
      typed_bin_op v2 (v2 ◁ᵥ n @ int size_t) p (p ◁ₗ{β} padded ty lyty ly) (PtrOffsetOp u8) (IntOp size_t) PtrOp T.
  Proof.
    ...
    by iPureIntro.
  Qed.
  Global Instance type_add_padded_inst v2 β ly lyty ty (p : loc) n:
    TypedBinOp v2 (v2 ◁ᵥ n @ int size_t)%I p (p ◁ₗ{β} padded ty lyty ly)%I (PtrOffsetOp u8) (IntOp size_t) PtrOp :=
    λ T, i2p (type_add_padded v2 β ly lyty ty T p n).
