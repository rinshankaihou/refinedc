...
Section bitfield_raw.
  ...
  Program Definition bitfield_raw_inner_type (it : int_type) (bv : Z) : type := {|
  ...
  Definition bitfield_raw (it : int_type) : rtype := {|
    rty_type := Z;
    rty := bitfield_raw_inner_type it;
  |}.

  Global Program Instance rmovable_bitfield_raw it : RMovable (bitfield_raw it) := {|
    rmovable bv := {|
...
Typeclasses Opaque bitfield_raw_inner_type.
...
Section bitfield.
  ...
  Program Definition bitfield_inner_type (R : Type) `{BitfieldDesc R} (bv : R) : type := {|
  ...
  Definition bitfield (R : Type) `{BitfieldDesc R} : rtype := {|
    rty_type := R;
    rty := bitfield_inner_type R;
  |}.

  Global Program Instance rmovable_bitfield R `{BitfieldDesc R} : RMovable (bitfield R) := {|
    rmovable bv := {|
...
Typeclasses Opaque bitfield_inner_type.
...
Section programs.
  ...
  Lemma type_not_bitfield_raw bv it v T:
    ...
    (⌜bv ∈ it⌝ -∗ T (i2v bv' it) (t2mt (bv' @ bitfield_raw it))) -∗
